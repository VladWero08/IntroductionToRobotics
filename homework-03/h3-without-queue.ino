/*
  ...
  Circuit components:
  - INPUT:
    - DIGITAL (PWM~) 9:  button that calls the elevator on level 0 
    - DIGITAL (PWM~) 10: button that calls the elevator on level 1
    - DIGITAL (PWM~) 11: button that calls the elevator on level 2
  - OUTPUT
    - DIGITAL 2:        led that is ON when the elevator is at level 0
    - DIGITAL (PWM~) 3: led that is ON when the elevator is at level 0
    - DIGITAL 4:         led that is ON when the elevator is at level 0

    Created 28-10-2023
    By Olaeriu Vlad Mihai
*/

// each level of the elevator will have an LED connected to a PIN,
// this array holds the PINS used for this elevator as:
// level 0 has PIN 2: elevator_level_LED_PINS[0] = 2 
const int elevator_level_LED_PINS[] = {2, 3, 4};
byte elevator_level_LED_states[] = {HIGH, LOW, LOW};
...
// about the button on each level of the elevator
struct elevator_button{
  const int button_PIN;                 // the PIN to which the button is connected
  int level;                            // the elevator level assigned to the button 
...
// initialize buttons for every level of the elevator
elevator_button elevator_level_0_button = {9,  0, LOW, LOW, 0};
elevator_button elevator_level_1_button = {10, 1, LOW, LOW, 0};
elevator_button elevator_level_2_button = {11, 2, LOW, LOW, 0};
...
int elevator_current_level = 0;
int elevator_destination_level = 0;
// depending on the destination level, the variable 
...
// move to the next level or to
// close/open its doors
const int elevator_actions_interval = 2000;

void setup() {
  // put your setup code here, to run once:
  pinMode(elevator_level_LED_PINS[0], OUTPUT);
  pinMode(elevator_level_LED_PINS[1], OUTPUT);
  pinMode(elevator_level_LED_PINS[2], OUTPUT);
  pinMode(elevator_moving_LED_PIN, OUTPUT);

  pinMode(elevator_level_0_button.button_PIN, INPUT_PULLUP);
  pinMode(elevator_level_1_button.button_PIN, INPUT_PULLUP);
  pinMode(elevator_level_2_button.button_PIN, INPUT_PULLUP);
...
void loop() {
  ...
  elevator_button_handler(elevator_level_0_button);
  elevator_button_handler(elevator_level_1_button);
  elevator_button_handler(elevator_level_2_button);

  elevator_tries_closing_doors();
  elevator_tries_moving();

  // write digital signals to each LED
  digitalWrite(elevator_level_LED_PINS[0], elevator_level_LED_states[0]);
  digitalWrite(elevator_level_LED_PINS[1], elevator_level_LED_states[1]);
  digitalWrite(elevator_level_LED_PINS[2], elevator_level_LED_states[2]);
...
/*
  ...
  If the button is active, it tries to move the elevator to the desired level.
*/
  ...
  if((millis() - elevator_button_.last_debounce_time) > debounce_delay){
    ...
    if(elevator_button_state != elevator_button_.current_button_state){
      ...
      if(elevator_button_.current_button_state == HIGH){
        ...
        elevator_level_change_handler(elevator_button_.level);
      }
    } 
  }
...
/*
  ...
  If it is not, the destination level of the elevator will be `destination_level`
  parameter received in the function.
*/
void elevator_level_change_handler(int destination_level){
  // if the elevator is moving, ignore the desired change 
  if(elevator_is_moving == HIGH){
    return;
  }

  // if the elevator is already on the desired level
  if(destination_level == elevator_current_level){
    return;
  }  

  if(destination_level < 0 || destination_level > 2){
    Serial.print("The elevator has only 3 levels! [0, 1, 2]. Level ");
    Serial.print(destination_level);
  ...
  elevator_destination_level = destination_level;

  // update the direction of the elevator
  if(elevator_destination_level > elevator_current_level)
...
void elevator_tries_moving(){
    if(elevator_is_moving == HIGH){  
      if((millis() - elevator_last_movement_time) > elevator_actions_interval){
        ...
        // it means it has reached the next level
        elevator_last_movement_time = millis();

        // the passed level LED will be turned off
        elevator_level_LED_states[elevator_current_level] = LOW;
        elevator_current_level += elevator_direction;
        // the newly reached level LED will be turned on 
        elevator_level_LED_states[elevator_current_level] = HIGH;
        
        if(elevator_current_level == elevator_destination_level){
